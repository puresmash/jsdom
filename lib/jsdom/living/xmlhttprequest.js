...
module.exports = function createXMLHttpRequest(window) {
  ...
  function setDispatchProgressEvents(xhr) {
    ...
    client.on("request", req => {
      // console.log('====onClientReq====')
      // const ct = req.getHeader('content-type')
      const cl = req.getHeader('content-length')
      // xhrUtils.updateRequestHeader(client.headers, "content-type", ct);
      // xhrUtils.updateRequestHeader(client.headers, "content-length", cl);
      let intervalId;
      if(cl) {
        lengthComputable = true;
        total = cl;
        const fn = () => {
          // let loaded = client.req.connection._bytesDispatched;
          let loaded = client.req.connection.bytesWritten;
          ...
          if(total === loaded) clearInterval(intervalId);
